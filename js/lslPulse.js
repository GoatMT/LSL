...
function buildWeeklyPulse(allData = []) {
  const data = [...allData]
    .filter((season) => (season.matches || []).some((match) => Number.isFinite(match.homeScore) && Number.isFinite(match.awayScore)))
    .sort((a, b) => Number(b.year) - Number(a.year))[0];
  if (!data) return null;

  const completed = (data.matches || []).filter((match) => Number.isFinite(match.homeScore) && Number.isFinite(match.awayScore));
  const latestWeek = Math.max(0, ...completed.filter((match) => match.stage === "regular").map((match) => Number(match.week) || 0));
  const weekMatches = completed.filter((match) => match.stage === "regular" && Number(match.week) === latestWeek);
  const standings = calculateStandings(data, { division: "Seniors" });
  const topTeam = standings[0];
  const topScorer = computePlayerStats(data, { stage: "regular" }).filter((player) => player.gamesPlayed > 0).sort((a, b) => b.goals - a.goals || b.wins - a.wins)[0];
  const spotlight = [...weekMatches].sort((a, b) => (b.homeScore + b.awayScore) - (a.homeScore + a.awayScore))[0];
  const teams = new Map((data.teams || []).map((team) => [team.id, team.shortName || team.name]));
  return { season: data.year, latestWeek, topTeam, topScorer, spotlight, teams };
}

function renderWeeklyPulse() {
  const pulse = state.leaguePulse;
  if (!pulse) return "";
  const spotlightTeams = pulse.spotlight ? `${pulse.teams.get(pulse.spotlight.homeTeamId) || "Home"} ${pulse.spotlight.homeScore}-${pulse.spotlight.awayScore} ${pulse.teams.get(pulse.spotlight.awayTeamId) || "Away"}` : "No completed match yet";
  return `
    <section class="section-panel pulse-weekly-panel">
      <div class="section-head">
        <div><span class="eyebrow">Weekly League Pulse</span><h2>${escapeHTML(pulse.season)} Week ${escapeHTML(pulse.latestWeek || "TBA")}</h2><p>A quick read on the latest completed regular-season week.</p></div>
        <a class="text-link" href="./matches.html">Open Match Center</a>
      </div>
      <div class="pulse-weekly-grid">
        <article><span class="eyebrow">Table Leader</span><strong>${escapeHTML(pulse.topTeam?.team?.shortName || pulse.topTeam?.team?.name || "TBA")}</strong><small>${pulse.topTeam ? `${pulse.topTeam.pts} pts | ${pulse.topTeam.w}-${pulse.topTeam.d}-${pulse.topTeam.l}` : "Standings pending"}</small></article>
        <article><span class="eyebrow">Top Scorer</span><strong>${escapeHTML(pulse.topScorer?.name || "TBA")}</strong><small>${pulse.topScorer ? `${pulse.topScorer.goals} goals | ${pulse.topScorer.gamesPlayed} GP` : "Scoring pending"}</small></article>
        <article><span class="eyebrow">Week Spotlight</span><strong>${escapeHTML(spotlightTeams)}</strong><small>Highest-scoring completed game in the latest week.</small></article>
      </div>
    </section>
  `;
}
...
async function init() {
  ...
  state.leaguePulse = buildWeeklyPulse(await loadAllSeasons().catch((error) => {

...
function teamAwardBadgeLabel(category = "") {
  if (/champion/i.test(category)) return "Champion";
  if (/2nd place|runner[- ]?up/i.test(category)) return "Runner-Up";
  if (/3rd place/i.test(category)) return "Third Place";
  if (/team mvp/i.test(category)) return "Team MVP";
  if (/golden boot/i.test(category)) return "Golden Boot";
  return category;
}

function renderTeamAchievementBadges(allData, team) {
  const awards = getAwards(allData, { season: "All", division: "All" }).filter((award) => award.teamId === team.id);
  if (!awards.length) return "";
  const trophyCount = awards.filter((award) => award.category === "Champion Team").length;
  const badges = awards
    .map((award) => `${teamAwardBadgeLabel(award.category)} | ${award.season}`)
    .filter(Boolean);
  return `
    <article class="card team-overview-card team-honors-card">
      <span class="eyebrow">Honors & Records</span>
      <div class="official-achievement-heading">
        <h3>Club Achievement Badges</h3>
        <span class="profile-trophy-count"><strong>${trophyCount}</strong> ${trophyCount === 1 ? "Trophy" : "Trophies"}</span>
      </div>
      <div class="profile-achievement-badge-list">
        ${badges.map((badge) => `<span class="profile-achievement-badge team-badge">${escapeHTML(badge)}</span>`).join("")}
      </div>
    </article>
  `;
}
  ...
  root.innerHTML = `
    <section class="section-panel team-profile-hero">
      ...
      <div class="team-overview-grid" id="overview" data-team-panel="overview">
        ...
        ${renderTeamAchievementBadges(allData, team)}

...
import { renderHomeFacts, hydrateHomeFacts } from "./homeFacts.js?v=4";
...
function registerPWA() {
  ...
  if ("serviceWorker" in navigator) {
    ...
    // Apply a newly published worker automatically so users do not need a hard refresh.
    ...
    window.addEventListener("load", async () => {
      try {
    const registration = await navigator.serviceWorker.register("./service-worker.js?v=20260914-1", {
          updateViaCache: "none",
        });
    ...
    <section class="section-panel important-news-panel">
      <div class="section-head compact-head">
        <div>
          ...
          <p>Championship results, major records, and the latest league news, all in one place.</p>
        </div>
...
function renderExpectedDates() {
  const dates = [
    { label: "Combine Day Expected", date: "May 24, 2027" },
    { label: "First Week", date: "Saturday Morning, June 5, 2027" },
    { label: "Get Your Jerseys", date: "Wednesday, June 2, 2027" },
    { label: "Teams", date: "Wednesday, June 2, 2027" },
  ];

  return `
    <section class="section-panel home-expected-dates" aria-labelledby="home-expected-dates-title">
      <div class="section-head compact-head">
        <div>
          <span class="eyebrow">Looking Ahead</span>
          <h2 id="home-expected-dates-title">2027 Expected Dates</h2>
          <p>Early planning dates for the next LSL season.</p>
        </div>
        <span class="pill">Planning</span>
      </div>
      <div class="home-expected-grid">
        ${dates
          .map(
            (item) => `
              <article class="home-expected-card">
...
function renderLanternRushSpotlight() {
  return `

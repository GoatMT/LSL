...
import { calculateTeamRecord, computeCombinedPlayerStats, computePlayerStats, playersWithOVR } from "./leagueEngine.js?v=3.12";
...
function yearsLabel(years = []) {
  return years.length ? [...years].sort((a, b) => Number(a) - Number(b)).join(", ") : "Not listed";
}

function teamsLabel(teams = []) {
  const labels = unique(teams.filter(Boolean).map((team) => teamCodeForName(team)));
  return labels.length ? labels.join(" / ") : "Team TBA";
}
...
function teamCodeForName(name) {
  return teamCodeMap.get(normalizeTeamName(name)) || fallbackTeamCode(name);
}
...
function stageLabel() {
  return stageOptions.find((option) => option.value === state.stage)?.label || "All";
}

function divisionLabel() {
  return divisionOptions.find((option) => option.value === state.division)?.label || "All";
}
...
function buildAllTimeTeams(seasons) {
  ...
  seasons.forEach((season) => {
    (season.teams || []).filter(matchesDivision).forEach((team) => {
      ...
      row.teamCode = teamCodeForTeam(team);
      row.seasons = unique([...row.seasons, season.year]).sort((a, b) => Number(a) - Number(b));
  ...
  return teams
    .map(
      (team, index) => `
        <tr>
          <td data-label="Rank">${index + 1}</td>
          <td data-label="Team"><a title="${escapeHTML(team.name)}" href="${escapeHTML(teamProfileHref(team.id, team.seasons?.at(-1) || ""))}">${escapeHTML(team.teamCode || teamCodeForName(team.name))}</a></td>
...
function render(allData, aliases) {
  ...
  root.innerHTML = `
    <section class="section-panel records-hero all-time-hero">
      <div class="records-hero-copy">
        <span class="eyebrow">LSL Archive</span>
        <h1>All Time Stats</h1>
        <p>Career leaders, team history, and season totals from every LSL campaign.</p>
        <div class="records-archive"><span>Season archive</span>${[...new Set(allData.map((season) => season.year))].sort((a, b) => Number(a) - Number(b)).map((year) => `<strong>${escapeHTML(year)}</strong>`).join("")}</div>
      </div>
      <div class="all-time-hero-actions">
        <img class="records-seal" src="${escapeHTML(SITE.logo)}" alt="Lantern Soccer League">
        <a class="button primary" href="./records.html">Open LSL Records</a>
      </div>
    </section>

    <section class="section-panel records-controls all-time-filter-card">
      <div class="section-head compact-head">
        <div>
          <span class="eyebrow">Archive Filters</span>
          <h2>Filter the leaderboard</h2>
          <p>Choose the people, teams, season, division, and games you want to compare.</p>
        </div>
      </div>
    ...
    <section class="section-panel all-time-results-card records-results-panel">

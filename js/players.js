...
import { computePlayerStats, computeCombinedPlayerStats, playersWithOVR } from "./leagueEngine.js?v=3.13";
...
function teamCode(team = {}) {
  ...
  const existing = String(team.shortName || team.abbreviation || "").trim();
  ...
  const initialsCode = String(team.name || "")
    ...
    .join("")
    .toUpperCase();
  return initialsCode.slice(0, 3) || "TBA";
}
...
function aggregatePlayers(allData) {
    ...
    computePlayerStats(season, { stage: state.stage })
      ...
      .forEach((player) => {
        ...
        if (player.teamId || player.teamName) {
          const seasonTeam = (season.teams || []).find((team) => team.id === player.teamId);
          current.teamHistory.push({
            id: player.teamId || "",
            name: player.teamName || "Team coming soon",
            shortName: teamCode(seasonTeam || { id: player.teamId, name: player.teamName }),
            year: season.year,
          });
        }
  ...
  return [...map.values()].map((player) => ({
    ...
    goalsPerGame: player.gamesPlayed ? player.goals / player.gamesPlayed : 0,
    winsPerGame: player.gamesPlayed ? player.wins / player.gamesPlayed : 0,
...
function sortPlayers(players) {
  ...
  const tieBreakers = {
    ...
    goalsPerGame: ["goalsPerGame", "goals", "wins", "ovr"],
    championships: ["championships", "goals", "wins", "ovr"],
    ovr: ["ovr", "goals", "wins"],
    gamesPlayed: ["gamesPlayed", "goals", "wins", "ovr"],
    wins: ["wins", "goals", "ovr"],
    winsPerGame: ["winsPerGame", "wins", "goals", "ovr"],
    ties: ["ties", "wins", "goals", "ovr"],
    losses: ["losses", "goals", "wins", "ovr"],
...
function filteredPlayers(allData) {
  ...
  const players = aggregatePlayers(allData)
    ...
    .filter((player) => player.gamesPlayed >= min)
    .filter((player) => {
...
function teamLabel(player) {
  const codes = unique(player.teamHistory.map((team) => team.shortName || teamCode(team)));
  if (!codes.length) return teamCode({ id: player.teamId, name: player.teamName });
  return codes.join(" / ");
}

function metricValue(player) {
  const value = Number(player[state.metric]) || 0;
  return ["goalsPerGame", "winsPerGame"].includes(state.metric) ? value.toFixed(2) : value;
}

function gameRangeLabel() {
  const min = Math.max(0, Number(state.minGames) || 0);
  return min ? `${min}+ games` : "Any games played";
}
          ...
          .map(
            (option) => `
              <button
                ...
                aria-label="${escapeHTML(option.label)}: ascending"
...
function metricCell(player, metric, label) {
  ...
  const numericValue = Number(player[metric]) || 0;
  const displayValue = ["goalsPerGame", "winsPerGame"].includes(metric) ? numericValue.toFixed(2) : numericValue;
  const value =
    metric === "ovr"
...
function leaderboardTable(players) {
  ...
  const showSelectedMetric = !["goals", "ovr"].includes(state.metric);
  const selectedMetricLabel = metricLabel();
      ...
      <table class="players-rank-table">
        ...
        <thead>
          <tr>
            ...
            <th scope="col" class="numeric${state.metric === "gamesPlayed" ? " active-metric" : ""}">GP</th>
            <th scope="col" class="numeric${state.metric === "goals" ? " active-metric" : ""}">Goals</th>
            ${showSelectedMetric ? `<th scope="col" class="numeric active-metric">${escapeHTML(selectedMetricLabel)}</th>` : ""}
            ...
            .map(
              (player, index) => `
                <tr>
                  ...
                  ${metricCell(player, "gamesPlayed", "GP")}
                  ${metricCell(player, "goals", "Goals")}
                  ${showSelectedMetric ? metricCell(player, state.metric, selectedMetricLabel) : ""}
...
function render(allData, focusId = "") {
  ...
  const min = Math.max(0, Number(state.minGames) || 0);
  const players = filteredPlayers(allData);

...
function collectMatchScorers(match, players) {
  ...
  for (const scorer of match.scorers || []) {
    ...
    scorers.set(scorer.playerId, {
      name: players.get(scorer.playerId)?.name || scorer.name,
      goals: goals + (previous?.goals || 0),
    });
  }
  return scorers;
}
...
export function buildHomeFacts(allData = []) {
  ...
  for (const season of allData) {
    ...
    for (const matches of groups.values()) {
      ...
      const scorersByMatch = new Map(matches.map(({match}) => [match.id, collectMatchScorers(match, players)]));
      const biggestSingleGameTotal = Math.max(
        0,
        ...[...scorersByMatch.values()].flatMap((scorers) => [...scorers.values()].map((scorer) => scorer.goals))
      );
      ...
      for (const entry of matches) {
        ...
        // Only display genuinely extreme team results, never ordinary score totals.
        if (total === mostGoals && total >= 6) {
        ...
        // A two-goal margin is a normal result, not a homepage record.
        if (margin === biggestMargin && margin >= 4) {
        ...
        const scorers = scorersByMatch.get(match.id) || new Map();
        if ([...scorers.values()].reduce((sum, scorer) => sum + scorer.goals, 0) > total) continue;
        for (const [id, scorer] of scorers) {
          // Only show the highest single-game scorer for the scope, not every 4-goal game.
          if (!scorer.name || scorer.goals < 4 || scorer.goals !== biggestSingleGameTotal || scorer.goals > Math.max(match.homeScore, match.awayScore)) continue;
